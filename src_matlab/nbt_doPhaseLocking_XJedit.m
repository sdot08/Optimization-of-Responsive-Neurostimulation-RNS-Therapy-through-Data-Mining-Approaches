...
function PhaseLockingObject = nbt_doPhaseLocking_XJedit(Signal,Fs,FrequencyBand,interval,filterorder,windowleng,overlap,indexPhase)
...
%% Calculate phase locking indices?
...
if(0) % FIR filter is only appropriate for very long signals
...
[b,a] = butter(2, FrequencyBand/(Fs/2));
...
phase = phase(perc10w+1:end-perc10w,:);
...
if ~isempty(windowleng)
    Nwind = round(windowleng*Fs);% 500msec*FS
...
%             disp([' channels ', num2str(k), ' ,' num2str(j), '...'])
            ...
            while (pos+nw-1 <= nx)                       % while enough signal left
                ...
                phase1w = phase1w(perc10w+1:end-perc10w);
                phase2w = phase2w(perc10w+1:end-perc10w);
                    
                    try
                        RP=n*phase1w-m*phase2w;
                        P_L_V(jump)=abs(sum(exp(1i*RP)))/length(RP);
                        if(calc_idx)
                            [ind1nm(jump),ind2nm(jump),ind3nm(jump)] = nbt_n_m_detection_XJedit(phase1w,phase2w,n,m);                    
                        end
...
%            disp([' channels ', num2str(k), ' ,' num2str(j), '...'])
            ...
            try 
               ...
               PLV(k,j)=abs(sum(exp(1i*RP)))/length(RP);
               if(calc_idx)
