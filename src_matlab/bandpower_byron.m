...
elseif nargin == 2
    ...
    % XJ edit 10/24/2018
    % Reason W(end)=124.8835, freqrange(2)=124.90; out of range due to "rounding error"
%     n = size(x,1); % This is the ORIGINAL code
    n = 2048; % make it fixed
    winsize = size(x,1);
    
    if isreal(x)
        [Pxx, F] = periodogram(x, hamming(winsize), n, fs);
    else
        [Pxx, F] = periodogram(x, hamming(winsize), n, fs, 'centered');
    end
